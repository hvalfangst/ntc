...
pub struct TaxCalculationInput {
    ...
    pub municipal_tax_rate: f64, // Default: 10% (varies by municipality)
    pub county_tax_rate: f64,    // Default: 11.4% (varies by county)
    pub church_tax_rate: f64,    // Default: 1.3% (optional)
    ...
    pub investment_wealth: f64, // For wealth tax calculation
    // Business expense deductions (for ENK)
...
pub enum EntityType {
    Individual,
    Corporation,
    Partnership, // Deltakerlignet selskap (LLC equivalent)
    SoleProprietorship, // ENK - Enkeltpersonforetak
}
...
pub struct TaxCalculationResult {
    ...
    pub investment_tax: f64, // Tax on dividends and capital gains
    pub wealth_tax: f64,     // Wealth tax on assets
    pub total_tax: f64,
    pub net_income: f64,
...
impl NorwegianTaxCalculator {
    ...
    fn calculate_individual_tax(input: &TaxCalculationInput) -> TaxCalculationResult {
        ...
        // Church tax (kirkeskatt) - only if church member
        ...
        // State tax (statsskatt) - progressive brackets
    ...
    fn calculate_corporate_tax(input: &TaxCalculationInput) -> TaxCalculationResult {
        ...
        // Corporate tax (selskapsskatt) - flat rate
        ...
        // Corporations have different investment tax rules (participation exemption)
        ...
        TaxCalculationResult {
            ...
            investment_tax,
            wealth_tax: 0.0, // Corporations don't pay wealth tax
            total_tax,
            net_income,
            effective_tax_rate,
            breakdown,
    ...
    fn calculate_partnership_tax(input: &TaxCalculationInput) -> TaxCalculationResult {
        // Partnership (deltakerlignet selskap) - taxed as individual but with different rules
        // Simplified: treating as individual for this implementation
        // In reality, partnerships have more complex rules regarding profit distribution
        let mut result = Self::calculate_individual_tax(input);
        
        // Add note about partnership taxation
    ...
    fn calculate_enk_tax(input: &TaxCalculationInput) -> TaxCalculationResult {
        ...
        // ENK (sole proprietorship) has no personal allowance like individuals
        ...
        // National insurance for ENK (higher rate: 10.9%)
    ...
    fn calculate_investment_tax(input: &TaxCalculationInput, breakdown: &mut Vec<TaxBreakdownItem>) -> f64 {
        ...
        // Calculate risk-free return allowance
    ...
    fn calculate_corporate_investment_tax(input: &TaxCalculationInput, breakdown: &mut Vec<TaxBreakdownItem>) -> f64 {
        // Corporations benefit from participation exemption
        // Only 3% of dividends are taxable at corporate rate (22%)
        let total_investment_income = input.dividend_income + input.capital_gains;
        ...
        let taxable_portion = total_investment_income * 0.03; // Only 3% is taxable
        ...
        if investment_tax > 0.0 {
            breakdown.push(TaxBreakdownItem {
                ...
                amount: investment_tax,
                rate: Some(0.66), // Effective rate: 3% * 22% = 0.66%
            });
        }
    ...
    fn calculate_wealth_tax(input: &TaxCalculationInput, breakdown: &mut Vec<TaxBreakdownItem>) -> f64 {
        ...
        // Apply 20% valuation discount for shares
